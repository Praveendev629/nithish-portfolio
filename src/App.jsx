...
import { Menu, X, Lock, Send, Smartphone, MessageSquare, Image as ImageIcon, Video, Github, Linkedin, Instagram, ExternalLink, Sparkles } from 'lucide-react';
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-8">
        {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
          <motion.div 
            ...
            whileHover={{ y: -10 }}
            className="aspect-[3/4] glass-card rounded-3xl overflow-hidden group relative cursor-pointer"
          >
            <div className="absolute inset-0 bg-zinc-900/50 group-hover:bg-purple-900/20 transition-all duration-500" />
            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-4 group-hover:translate-y-0">
               <div className="p-4 bg-white/10 backdrop-blur-md rounded-2xl border border-white/20">
                 {i % 2 === 0 ? <Video size={24} className="text-white" /> : <ImageIcon size={24} className="text-white" />}
               </div>
            </div>
          </motion.div>
        ))}
